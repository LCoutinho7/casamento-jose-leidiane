    ...
    <section id="presentes" className="bg-sand/50 px-6 py-24 md:px-10 md:py-32">
      ...
      <ul className="mx-auto mt-6 grid max-w-5xl gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {GIFT_QUOTAS.map((gift) => (
          <li key={gift.id} className="flex flex-col justify-between rounded-2xl border border-olive/15 bg-paper p-6 shadow-paper transition-[border-color,transform] duration-150 ease-out-soft hover:-translate-y-0.5 hover:border-olive/40">
            <div>
            ...
            <div className="mt-6 flex items-center justify-between gap-3 border-t border-olive/15 pt-4">
              <span className="font-mono text-lg text-olive">{gift.customAmount ? 'Você escolhe' : formatCurrency(gift.price)}</span>
