  ...
  return (
    <section id="inicio" className="relative flex min-h-svh items-end overflow-hidden bg-ink text-paper">
      ...
      <div className="relative mx-auto w-full max-w-6xl px-6 pt-40 pb-20 md:px-10 md:pb-28">
        <p className="eyebrow mb-5 flex animate-reveal-up items-center gap-3 text-sand motion-reduce:animate-none">
          <span className="h-px w-8 bg-sand/60" />
          Vamos nos casar
        </p>

        <h1 className="max-w-4xl animate-reveal-mask font-serif text-6xl leading-[0.95] tracking-tight text-balance italic motion-reduce:animate-none sm:text-7xl md:text-8xl lg:text-[7.5rem]" style={{ animationDelay: '120ms' }}>
          {COUPLE.groom} <span className="font-normal not-italic">&amp;</span> {COUPLE.bride}
        </h1>

        <p
          className="mt-6 animate-reveal-up font-mono text-sm tracking-[0.3em] text-sand uppercase motion-reduce:animate-none md:text-base"
          style={{ animationDelay: '520ms' }}
        >
          {WEDDING_DATE_LABEL}
        </p>

        <div className="mt-10 flex animate-reveal-up flex-wrap items-center gap-4 motion-reduce:animate-none" style={{ animationDelay: '680ms' }}>
