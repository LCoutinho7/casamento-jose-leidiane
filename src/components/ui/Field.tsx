...
const control =
  'w-full rounded-xl border border-olive/30 bg-white px-4 py-2.5 text-sm text-ink placeholder:text-olive/50 transition-colors focus:border-olive focus:outline-none';
  ...
  return (
    <div>
      <label htmlFor={inputId} className="mb-1.5 flex items-baseline justify-between text-xs font-semibold tracking-wider text-muted uppercase">
        {label}
        {hint && <span className="font-mono text-[0.6875rem] font-normal normal-case text-olive">{hint}</span>}
      </label>
